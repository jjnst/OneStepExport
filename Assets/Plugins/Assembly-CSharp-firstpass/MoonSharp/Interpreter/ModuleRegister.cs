		...
		public static Table RegisterModuleType(this Table gtable, Type t)
		{
				...
				select __mi)
			{
				if (item.GetCustomAttributes(typeof(MoonSharpModuleMethodAttribute), false).ToArray().Length != 0)
				{
					...
					Delegate @delegate = Delegate.CreateDelegate(typeof(Func<ScriptExecutionContext, CallbackArguments, DynValue>), item);
					Func<ScriptExecutionContext, CallbackArguments, DynValue> callBack = (Func<ScriptExecutionContext, CallbackArguments, DynValue>)@delegate;
