...
namespace MoonSharp.Interpreter.Interop
{
	public class StandardUserDataDescriptor : DispatchingUserDataDescriptor, IWireableDescriptor
	{
		...
		private void FillMemberList()
		{
			...
			{
				if (!hashSet.Contains(type2.Name) && !Framework.Do.IsGenericTypeDefinition(type2) && (Framework.Do.IsNestedPublic(type2) || Framework.Do.GetCustomAttributes(type2, typeof(MoonSharpUserDataAttribute), true).Length != 0))
				{
					IUserDataDescriptor userDataDescriptor = UserData.RegisterType(type2, AccessMode);
