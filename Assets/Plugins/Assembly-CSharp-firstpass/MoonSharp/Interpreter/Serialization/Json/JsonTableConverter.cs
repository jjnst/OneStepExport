...
namespace MoonSharp.Interpreter.Serialization.Json
{
	public static class JsonTableConverter
	{
		...
		{
			DynValue dynValue = ObjectValueConverter.SerializeObjectToDynValue(null, obj, JsonNull.Create());
			return dynValue.Table.TableToJson();
		}
		...
		private static DynValue ParseJsonValue(Lexer L, Script script)
		{
			...
			{
				Table table = ParseJsonObject(L, script);
				return DynValue.NewTable(table);
			}
			if (L.Current.Type == TokenType.Brk_Open_Square)
			{
				Table table2 = ParseJsonArray(L, script);
