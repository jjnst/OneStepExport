...
namespace MoonSharp.Interpreter.Execution.VM
{
	internal sealed class Processor
	{
		...
		private Stack<Processor> deadProceys = new Stack<Processor>();

		private Processor m_Processor;

		private const int YIELD_SPECIAL_TRAP = -99;

		internal long AutoYieldCounter = 0L;
		...
		internal Processor(Script script, Table globalContext, ByteCode byteCode)
		{
		...
		private void EnterProcessor()
		{
			...
			{
				string message = string.Format("Cannot enter the same MoonSharp processor from two different threads : {0} and {1}", m_OwningThreadID, threadId);
		...
		{
			if (deadProceys.Count == 0)
			{
				for (int i = 0; i < 10; i++)
				{
					deadProceys.Push(new Processor(this));
				}
			}
			Processor processor = deadProceys.Pop();
			processor.m_State = CoroutineState.NotStarted;
			processor.m_Processor = this;
		...
		public DynValue Coroutine_Resume(DynValue[] args)
		{
			...
			try
			{
				...
				m_Processor.deadProceys.Push(this);
				return dynValue;
			}
			catch (Exception)
			{
		...
		{
			List<Processor> source = ((m_Parent != null) ? m_Parent.m_CoroutinesStack : m_CoroutinesStack);
			return source.Select((Processor c) => new WatchItem
			{
		...
		private DynValue GetStoreValue(Instruction i)
		{
			...
			{
				return (numVal2 < dynValue.Tuple.Length) ? dynValue.Tuple[numVal2] : DynValue.NewNil();
			}
			return (numVal2 == 0) ? dynValue : DynValue.NewNil();
		}
