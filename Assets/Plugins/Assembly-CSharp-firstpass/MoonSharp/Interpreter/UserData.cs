...
namespace MoonSharp.Interpreter
{
	public class UserData : RefIdObject
	{
		...
		public static Table GetDescriptionOfRegisteredTypes(bool useHistoricalData = false)
		{
			...
			IEnumerable<KeyValuePair<Type, IUserDataDescriptor>> enumerable = (useHistoricalData ? TypeDescriptorRegistry.RegisteredTypesHistory : TypeDescriptorRegistry.RegisteredTypes);
			foreach (KeyValuePair<Type, IUserDataDescriptor> item in enumerable)
			{
		...
		{
			IEnumerable<KeyValuePair<Type, IUserDataDescriptor>> source = (useHistoricalData ? TypeDescriptorRegistry.RegisteredTypesHistory : TypeDescriptorRegistry.RegisteredTypes);
			return source.Select((KeyValuePair<Type, IUserDataDescriptor> p) => p.Value.Type);
		}
	}
}
