...
namespace Discord
{
	public class VoiceManager
	{
		...
		{
			events.OnSettingsUpdate = delegate
			{
		...
		{
			FFIMethods.SetInputModeCallback setInputModeCallback = delegate(IntPtr ptr, Result result)
			{
				Utility.Release(ptr);
				callback(result);
			};
			Methods.SetInputMode(MethodsPtr, inputMode, Utility.Retain(setInputModeCallback), setInputModeCallback);
		}
