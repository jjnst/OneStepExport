	...
	public class ControlRemappingDemo1 : MonoBehaviour
	{
		...
		private void DrawPlayerSelector()
		{
			...
			foreach (Player player in ReInput.players.GetPlayers(true))
			{
				...
				bool flag2 = GUILayout.Toggle(flag, (player.descriptiveName != string.Empty) ? player.descriptiveName : player.name, "Button", GUILayout.ExpandWidth(false));
				if (flag2 != flag && flag2)
				{
					selectedPlayer = player;
		...
		private void DrawMapCategories()
		{
			...
			foreach (InputMapCategory userAssignableMapCategory in ReInput.mapping.UserAssignableMapCategories)
			{
				...
				bool flag3 = GUILayout.Toggle(flag2, (userAssignableMapCategory.descriptiveName != string.Empty) ? userAssignableMapCategory.descriptiveName : userAssignableMapCategory.name, "Button", GUILayout.ExpandWidth(false));
				if (flag3 != flag2)
				{
					selectedMapCategoryId = userAssignableMapCategory.id;
					selectedMap = selectedPlayer.controllers.maps.GetFirstMapInCategory(selectedController.type, selectedController.id, userAssignableMapCategory.id);
